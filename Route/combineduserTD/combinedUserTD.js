...
const PostUserTrack = require('../../Route/Adviser/UserTrack/AddUserTrack');
const PostUserData = require('../../Route/UserData/AddLIHC'); // Adjust the path accordingly
...
async function main(req, res) {
    ...
    try {
        await session.withTransaction(async () => {
            ...
            let objOfLIHC_Data = req.body.LIHC_Data;
            // res.send( objOfLIHC_Data)

            const result1 = await PostUserTrack(objOfUserTracking, res, session);

            objOfLIHC_Data.UserTracking_FK =  result1._id;
            // console.log("after geting result1",result1)
            // let myemail={UserEmail:req.body.UserEmail};
            // console.log("myemail", myemail)
            const result2 = await PostUserData(objOfLIHC_Data, res, session);
